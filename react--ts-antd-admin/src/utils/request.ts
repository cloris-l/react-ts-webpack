...
import {apiBaseUrl} from '../config'; // @代表src一级目录，是我们在vue.config.js文件里配置的
// const { apiBaseUrl: { dev, pro } } = config;
const baseUrl = process.env.NODE_ENV === 'production' ? apiBaseUrl.pro : apiBaseUrl.dev;
...
class HttpRequest { // 定义一个接口请求类，用于创建一个axios请求实例
    constructor(public baseUrl: string = baseUrl) { // 这个类接收一个字符串参数，是接口请求的基本路径
